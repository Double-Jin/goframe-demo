...
var fileService = fileImpl{}

func File() sFile {
	return &fileService
}

func (s *fileImpl) UploadLocal(ctx context.Context, req *apiAdmin.UploadFileReq) (res *apiAdmin.UploadFileRes, err error) {
	files := req.File
...
func (s *fileImpl) UploadOss(ctx context.Context, req *apiAdmin.UploadFileReq) (res *apiAdmin.UploadFileRes, err error) {
	files := req.File
	names, err := files.Save("public/upload/")
	if err != nil {
		err = gerror.New("文件上传失败")
